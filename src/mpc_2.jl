...
# function init_x(m, circuit, demand)
    ...
    set_start_value.(m[:Vm], Vm)
    set_start_value.(m[:Va], Va)
    ...
    set_start_value.(m[:Pg], Pg)
    set_start_value.(m[:Qg], Qg)

    if opt.sc_constr
        for g=1:num_gens
            set_start_value.(m[:Pg_f][g], Pg[1,g])
            set_start_value.(m[:Qg_f][g], Qg[1,g])
        end

        for b=1:num_buses
            set_start_value.(m[:Vm_f][b], Vm[1,b])
            set_start_value.(m[:Va_f][b], Va[1,b])
        end
    end

    if opt.freq_ctrl
        for t=1:T, g in active_gen
            set_start_value.(m[:Pg_ref][t,g], Pg[g])
        end
        set_start_value.(m[:omega], zeros(T))
    end

    if opt.load_shed
        set_start_value.(m[:sigma_p], zeros(T, num_buses))
        set_start_value.(m[:sigma_m], zeros(T, num_buses))
    end
end
